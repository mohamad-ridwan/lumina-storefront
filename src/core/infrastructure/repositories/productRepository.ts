...
import { Product, ProductCollection, ProductSearchCriteria } from '@/core/domain/product';
...
export interface ProductRepository {
  ...
  getByCategory(categorySlug: string, page?: number, limit?: number): Promise<ProductCollection>;
}
...
class HttpProductRepository implements ProductRepository {
  async getById(id: string): Promise<Product> {
    const response = await httpClient.get<ApiProductResponse>(`/api/shoes/${id}`);
    return this.mapToProduct(response.data.data);
  }

  async getBySlug(slug: string): Promise<Product> {
    const response = await httpClient.get<ApiProductResponse>(`/api/shoes/slug/${slug}`);
    return this.mapToProduct(response.data.data);
  }
  ...
  async getLatestOffers(page = 1, limit = 12): Promise<ProductCollection> {
    const response = await httpClient.get<ApiProductsResponse>(
      `/api/shoes/latest-offers?page=${page}&limit=${limit}`
    );
    return this.mapToProductCollection(response.data);
  }

  async getByCategory(categorySlug: string, page = 1, limit = 12): Promise<ProductCollection> {
    const response = await httpClient.get<ApiProductsResponse>(
      `/api/shoes/category/${categorySlug}?page=${page}&limit=${limit}`
    );
    return this.mapToProductCollection(response.data);
  }
